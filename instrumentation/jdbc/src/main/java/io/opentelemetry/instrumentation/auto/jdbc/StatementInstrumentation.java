...
public final class StatementInstrumentation extends Instrumenter.Default {
  ...
  public static class StatementAdvice {
    ...
    public static void onEnter(
      ...
      System.out.println("#########1 StatementInstrumentation::StatementAdvice::onEnter");
      callDepth = TRACER.getCallDepth();
      if (callDepth.getAndIncrement() == 0) {
        span = TRACER.startSpan(statement, sql);
        if (span != null) {
          System.out.println("#########1 StatementInstrumentation span is NOT null");
          scope = TRACER.startScope(span);
        } else {
          System.out.println("#########1 StatementInstrumentation span is null somehow");
        }
      }
    }
    ...
    public static void stopSpan(
        ...
        @Advice.Local("otelScope") Scope scope,
        @Advice.Local("otelCallDepth") Depth callDepth) {
      System.out.println("#########1 StatementInstrumentation::StatementAdvice::stopSpan");
      if (callDepth.decrementAndGet() == 0 && scope != null) {
        scope.close();
        if (throwable == null) {
          TRACER.end(span);
          System.out.println("#########1 StatementInstrumentation::tracer.end span: " + span);

        } else {
          System.out.println("#########1 StatementInstrumentation::tracer.endExceptionally");
