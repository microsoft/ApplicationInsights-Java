...
package io.opentelemetry.instrumentation.auto.opentelemetryapi.v0_2_4.trace;
...
public class Bridging {
  ...
  public static Attributes toShaded(final Map<String, AttributeValue> unshadedAttributes) {
    Attributes.Builder shadedAttributes = Attributes.newBuilder();
    for (Map.Entry<String, AttributeValue> entry : unshadedAttributes.entrySet()) {
      AttributeValue value = entry.getValue();
      switch (value.getType()) {
        case BOOLEAN:
          shadedAttributes.setAttribute(entry.getKey(), value.getBooleanValue());
          break;
        case DOUBLE:
          shadedAttributes.setAttribute(entry.getKey(), value.getDoubleValue());
          break;
        case LONG:
          shadedAttributes.setAttribute(entry.getKey(), value.getLongValue());
          break;
        case STRING:
          shadedAttributes.setAttribute(entry.getKey(), value.getStringValue());
          break;
      }
    }
    return shadedAttributes.build();
  }
