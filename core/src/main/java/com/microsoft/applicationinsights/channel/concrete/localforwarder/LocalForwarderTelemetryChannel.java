...
public class LocalForwarderTelemetryChannel extends ATelemetryChannel<Telemetry> {
    ...
    private static <T extends BaseTelemetry> Telemetry.Builder telemetryBuilderWithStandardFields(T telemetry) {
        ...
        tb.setDataTypeName(BaseTelemetry.getTelemetryName(BaseTelemetry.normalizeInstrumentationKey(iKey), telemetry.getEnvelopName()));
        return tb;
    }

    private static Duration transformDuration(com.microsoft.applicationinsights.telemetry.Duration d) {
    ...
    static {
        ...
        transformers.put(MetricTelemetry.BASE_TYPE, new Function<BaseTelemetry, Telemetry>() {
            ...
            public Telemetry apply(BaseTelemetry bt) {
                ...
                // FIXME iterate metrics in MetricData?
    ...
    protected boolean doSend(com.microsoft.applicationinsights.telemetry.Telemetry telemetry) {
        ...
        final String typeName = base.getBaseTypeName();
        final Function<BaseTelemetry, Telemetry> transformer = transformers.get(typeName);
        if (transformer == null) {
            InternalLogger.INSTANCE.error("Could not find transformer for type='%s'", typeName);
            return false;
        }
        try {
            telemetryBuffer.add(transformer.apply(base));
            telemetry.reset();
            return true;
