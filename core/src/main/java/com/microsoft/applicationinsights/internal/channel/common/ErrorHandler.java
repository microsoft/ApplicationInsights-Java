...
/**
 * This class implements the retry logic for transmissions with the results of a 408, 500, and 503
 * result.
 *
 * <p>It does not handle any error codes such as 400, 401, 403, 404, etc.
 *
 * @author jamdavi
 */
public class ErrorHandler implements TransmissionHandler {
   ...
   * Ctor
   *
   * <p>Constructs the ErrorHandler object.
   *
  ...
  private void backoffAndSendTransmission(TransmissionHandlerArgs args) {
    ...
    if (args.getTransmission() != null
        && (args.getTransmission().getNumberOfSends()
            > transmissionPolicyManager.getMaxInstantRetries())) {
