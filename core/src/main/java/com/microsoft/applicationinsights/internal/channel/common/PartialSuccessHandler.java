...
/**
 * This class implements the retry logic for partially accepted transmissions. HTTP status code 206.
 *
 * <p>
 *
 * @see <a href=
...
public class PartialSuccessHandler implements TransmissionHandler {
   ...
   * Ctor
   *
   * <p>Constructs the PartialSuccessHandler object.
   *
   * @param policy The {@link TransmissionPolicyManager} object that is needed to control the back
   *     off policy.
   */
   ...
   *
   * @param args The {@link TransmissionHandlerArgs} for this transmission.
      ...
      switch (args.getResponseCode()) {
        case HttpStatus.SC_PARTIAL_CONTENT:
          ...
          if (backendResponse != null
              && (backendResponse.itemsAccepted < backendResponse.itemsReceived)) {
          ...
          InternalLogger.INSTANCE.trace(
              "Skipping partial content handler due to itemsAccepted and itemsReceived being equal.");
          return false;

        default:
          InternalLogger.INSTANCE.trace(
              "Http response code %s not handled by %s",
              args.getResponseCode(), this.getClass().getName());
          return false;
      }
    }
    return false;
  ...
  /**
   * Used to parse the original telemetry request in order to resend the failed ones.
   *
   * @param args The {@link TransmissionHandlerArgs} that contains the {@link Transmission} object.
    ...
    if (args.getTransmission().getWebContentEncodingType() == "gzip") {
      ...
      try {
        ...
        InternalLogger.INSTANCE.error(
            "IOException: Error while reading the GZIP stream.%nStack Trace:%n%s",
            ExceptionUtils.getStackTrace(ex));
      } catch (Throwable t) {
        InternalLogger.INSTANCE.error(
            "Error while reading the GZIP stream.%nStack Trace:%n%s",
        ...
        if (gis != null) {
          try {
            ...
            InternalLogger.INSTANCE.warn(
                "Error while closing the GZIP stream.%nStack Trace:%n%s",
                ExceptionUtils.getStackTrace(ex));
          }
        }
        if (bufferedReader != null) {
          try {
            ...
            InternalLogger.INSTANCE.warn(
                "Error while closing the buffered reader.%nStack Trace:%n%s",
                ExceptionUtils.getStackTrace(ex));
          }
        }
      }
  ...
  /**
   * Sends a new transmission generated from the failed attempts from the original request.
   *
   * @param args The {@link TransmissionHandlerArgs} object that contains the {@link
   *     TransmissionDispatcher}
   * @param newTransmission The {@link List} of items to resent
   ...
   * @return A {@link BackendResponse} object that contains the status of the partial success.
   */
