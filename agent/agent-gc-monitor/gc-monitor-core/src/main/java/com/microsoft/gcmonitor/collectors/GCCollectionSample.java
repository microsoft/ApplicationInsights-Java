...
class GCCollectionSample implements GCCollectionEvent {
    ...
    public GCCollectionSample(GarbageCollector collector, CompositeData collectionData, String gcCause, String gcAction, MemoryManagement memoryManagement) {
        ...
        memoryUsageBeforeGc = groupMemoryUsageByPoolName(collectionData.get(MEMORY_USAGE_BEFORE_GC), memoryManagement);
        memoryUsageAfterGc = groupMemoryUsageByPoolName(collectionData.get(MEMORY_USAGE_AFTER_GC), memoryManagement);
    ...
    private static MemoryUsage aggregateMemoryPools(List<MemoryPool> pools, Map<MemoryPool, MemoryUsage> pool) {
        return pools
                .stream()
                .map(pool::get)
                .reduce(new MemoryUsage(0, 0, 0, -1), (acc, value) -> {
                    long max;
    ...
    public List<MemoryPool> getYoungPools() {
        return memoryUsageAfterGc.keySet().stream().filter(MemoryPool::isYoungPool).collect(Collectors.toList());
    }
}
