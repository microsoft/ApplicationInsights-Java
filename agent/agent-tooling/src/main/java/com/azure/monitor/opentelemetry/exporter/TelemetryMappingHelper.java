...
public class TelemetryMappingHelper {
  ...
  static void addEventTelemetryItems(
      SpanData span,
    ...
    for (EventData event : span.getEvents()) {
      ...
      if (event.getAttributes().get(SemanticAttributes.EXCEPTION_TYPE) != null
        ...
        if (stacktrace != null) {
          addExceptionTelemetryItem(
              stacktrace, span, operationName, instrumentationKey, telemetryItems);
        }
        return;
      }

      MessageTelemetryBuilder telemetryBuilder = MessageTelemetryBuilder.create();
      initTelemetry(telemetryBuilder, instrumentationKey);
  ...
  static void addExceptionTelemetryItem(
      String errorStack,
      SpanData span,
      @Nullable String operationName,
      String instrumentationKey,
  ...
  static void addRemoteDependencyTelemetryItem(
      SpanData span,
      boolean inProc,
      String instrumentationKey,
      List<TelemetryItem> telemetryItems) {
    ...
    telemetryItems.add(telemetryBuilder.build());
    addEventTelemetryItems(span, null, instrumentationKey, telemetryItems);
  }

  static void addRequestTelemetryItem(
      SpanData span, String instrumentationKey, List<TelemetryItem> telemetryItems) {
    ...
    telemetryItems.add(telemetryBuilder.build());
    addEventTelemetryItems(span, operationName, instrumentationKey, telemetryItems);
  }
