...
public class CoreAndFilterTests extends AiSmokeTest {
  ...
  public void trackDependency() throws Exception {
    ...
    int expectedItems = 2;
    assertEquals(
        String.format("There were %d extra telemetry items received.", expectedItems - totalItems),
        expectedItems,
        totalItems);
  ...
  public void testAutoExceptionWithFailedRequest() {
    ...
    // due to there is a bug, the success for the request data is not correct, so just ignore this
    // case now.
