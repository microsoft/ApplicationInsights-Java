...
public class FixedAiTestCases {
  ...
  public Runnable getTrackEvent() {
    return new Runnable() {
      ...
      public void run() {
        tclient.trackEvent(
            "AiTestEvent", getPropertyMapForMethod("Event"), getMetricMapForMethod("Event"));
      }
    };
  }
    ...
    return new Runnable() {
      ...
      public void run() {
        tclient.trackMetric(
            ...
            123.4,
            567.8,
            getPropertyMapForMethod("Metric"));
      }
    };
  }
  ...
  public Runnable getTrackException() {
    return new Runnable() {
      @Override
      public void run() {
        tclient.trackException(
            new Exception("TestException", new Exception("TestExceptionCause")),
            getPropertyMapForMethod("Exception"),
            getMetricMapForMethod("Exception"));
      }
    };
  }
  ...
  public Runnable getTrackDependency() {
    return new Runnable() {
      @Override
      public void run() {
        tclient.trackDependency(
            "TestDependency", "TestCommand", new Duration(0, 0, 0, 12, 345), true);
      }
    };
  }
