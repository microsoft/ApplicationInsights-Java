...
public class AiDockerClient {
  ...
  public AiDockerClient(String user, String shellExecutor) {
    ...
    if (!de.exists()) {
      throw new SmokeTestException(
          String.format("Could not find docker: %s=%s", DOCKER_EXE_ENV_VAR, dockerPath));
    }
  ...
  public String startContainer(String image, String portMapping, String network)
      throws IOException, InterruptedException {
    ...
    List<String> cmd =
        new ArrayList<>(
            Arrays.asList(
                ...
                "-p",
                portMapping,
                "--add-host=fakeingestion:" + localIp));
  ...
  public void execOnContainer(String id, String cmd, String... args)
    ...
    try {
      waitAndCheckCodeForProcess(
          ...
          10,
          TimeUnit.SECONDS,
          String.format("executing command on container: '%s'", id, Joiner.on(' ').join(cmdList)));
      flushStdout(p);
    } catch (Exception e) {
      throw e;
    }
  ...
  public boolean isContainerRunning(String id) throws IOException, InterruptedException {
    Process p =
        new ProcessBuilder(dockerExePath, "inspect", "-f", "{{.State.Running}}", id).start();
    waitAndCheckCodeForProcess(
        p, 1, TimeUnit.SECONDS, String.format("checking if container is running: %s", id));
  ...
  public String createNetwork(String name) throws IOException, InterruptedException {
    Process p =
        buildProcess(dockerExePath, "network", "create", "--driver", "bridge", name).start();
  ...
  /**
   * Returns container name for a running container. If the container id is not running, it returns
   * null.
   */
  public String getRunningContainerName(String containerId)
      throws IOException, InterruptedException {
    Process p =
        buildProcess(dockerExePath, "inspect", "--format", "'{{.Name}}'", containerId).start();
