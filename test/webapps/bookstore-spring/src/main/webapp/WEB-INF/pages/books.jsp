...
<div>
    <table align="center" style="background-color: wheat; border:1px solid black; ; font-size: 24px"
            ...
            <tr bgcolor="${loopStatus.index % 2 == 0 ? '#aaaaaa' : '#cccccc'}">
                ...
                <c:choose>
                    <c:when test="${bookData.loaned == 'true'}">
                        <td align="center" bgcolor="#7fff00"><c:out
                                value="${bookData.loaned}"/></td>
                    </c:when>
                    <c:otherwise>
                ...
                <td align="center"><a
                        href='loan?title=${bookData.book.title}&id=<c:out value="${bookData.book.isbn10}"/>&subject=${subject}'>Borrow
                    this book!</a></td>
            </tr>
        </c:forEach>
